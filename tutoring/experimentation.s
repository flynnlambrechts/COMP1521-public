...
swap_loop__body:
	...
	lw	$t4, numbers($t1)		#   x = numbers[i];

	addi	$t3, $t1, -4			#   &numbers[i - 1] = &numbers[i] - 4
	lw	$t5, numbers($t3)		#   y = numbers[i - 1];

	bge	$t4, $t5, swap_loop__continue	#   if (x < y) {
	sw	$t5, numbers($t1)			#     numbers[i] = y;
	sw	$t4, numbers($t3)			#     numbers[i - 1] = x;
						#   }
swap_loop__continue:
	addi	$t0, $t0, 1	 		#   i++;
	j	swap_loop__cond			# }
