...
points_loop_body:
	...
	mul	$t5, $t0, SIZE_OF_POINT_2D 	# [i]
	add	$t4, $t4, $t5			# $t4 = &my_points[i]

	add	$t5, $t4, POINT_2D_ROW_OFFSET   # $t5 = &my_points[i].row
	lw	$t1, ($t5)			# row = my_points[i].row

	add	$t5, $t4, POINT_2D_COL_OFFSET   # $t5 = &my_points[i].col
	lw	$t2, ($t5)			# col = my_points[i].col;

	mul	$t4, $t1, MAP_SIZE
	add	$t4, $t4, $t2
	mul	$t4, $t4, 4		# $t4 = OFFSET ([row][col])
